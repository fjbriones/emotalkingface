...
for root, dirs, files in os.walk(h5_folder):
    for filename in files:
        if filename.endswith((".h5", ".hdf5")):
            ...
            h5_file_list.append(
                (os.path.join(root, filename), emotion, emotion_intensity, labels[0])
            )
...
cnt = 0
with torch.no_grad():
    for s_idx, (h5_file, emotion, emotion_intensity, s_id) in tqdm(
        enumerate(h5_file_list)
    ):
        ...
        for emotion in range(len(emotion_dict)):
            emo_label = (
                torch.from_numpy(to_categorical(emotion, num_classes=6))
                .float()
                .unsqueeze(0)
            )

            out, _ = generator(
                image_c.to(args.device),
                speech_t.to(args.device),
                emo_label.to(args.device),
            )
            ...
            utils.write_video_cv(
                ...
                speech[0, 0, :],
                8000,
                os.path.join(output_path, s_id),
                str(s_idx) + "_pd_" + emotion_dict[emotion] + ".mp4",
                25.0,
            )
