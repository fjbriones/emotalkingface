...
class TemplateProcessor:
    def __init__(self, path):
        ...
        shape = (
            (128 * (shape / np.tile(np.max(shape, axis=0), [68, 1]))) * scl
        ) + 128 * ((1 - scl) / 2)
...
parser.add_argument(
    "-is", "--speech-file", type=str, help="input speech file", default=None
)
parser.add_argument(
    "-im", "--img-file", type=str, help="input image file", default=None
)
parser.add_argument(
    "-ih", "--hdf5-file", type=str, help="input hdf5 file", default=None
)
...
parser.add_argument(
    ...
    type=str,
    help="Predictor Path",
    default="data/dlib_data/shape_predictor_68_face_landmarks.dat",
)
parser.add_argument(
    "-ti",
    "--temp-img",
    ...
    default="data_prep/template_face.jpg",
)
...
generator.load_state_dict(
    torch.load(
        os.path.join(args.model, "generator.pt"),
...
if args.hdf5_file:
    ...
    print(I.shape)
    if I.shape[0] == I.shape[1] and I.shape[1] == 128:
...
normTransform = A.Compose(
    [A.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5), always_apply=True)], p=1
)
...
with torch.no_grad():
    ...
    for emo_i in range(6):
        emo_label = (
            torch.from_numpy(to_categorical(emo_i, num_classes=6)).float().unsqueeze(0)
        )
        print(emo_label)
        out, _ = generator(
            image_c.to(args.device), speech_t.to(args.device), emo_label.to(args.device)
        )
        ...
        utils.write_video_cv(
            frame_list[: -args.offset, ...],
            speech[0, 0, : int(-args.offset * args.increment)],
            sr,
            output_path,
            emotion_dict[emo_i] + "_generated.mp4",
            args.fps,
        )
