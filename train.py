...
def initParams():
    ...
    parser.add_argument(
        ...
        "--in-path",
        type=str,
        help="Input folder containing train data",
        default=None,
        required=True,
    )
    parser.add_argument(
        ...
        "--val-path",
        type=str,
        help="Input folder containing validation data",
        default=None,
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-m", "--model", type=str, help="Pre-trained model path", default=None
    )
    parser.add_argument(
        "-mde",
        "--model_disc_emo",
    ...
    parser.add_argument(
        "--disc_frame_gp",
        type=float,
        help="Weight for gradient penalty of the frame discriminator.",
        default=10.0,
    )
    parser.add_argument(
        "--disc_emo_gp",
        type=float,
        help="Weight for gradient penalty of the emotion discriminator.",
        default=10.0,
    )
    ...
    parser.add_argument(
        "--disc_emo_weight",
        type=float,
        help="Weight for emotion losses in the emotion discriminator.",
        default=1000.0,
    )
...
def train():
    ...
    val_loader = torch.utils.data.DataLoader(
        valDset, batch_size=4, shuffle=True, drop_last=True, **args.kwargs
    )
    ...
    if args.model_disc_emo:
        disc_emo.load_state_dict(
            torch.load(
                os.path.join(args.model_disc_emo, "disc_emo.pt"),
                map_location="cuda" if args.cuda else "cpu",
            ),
            strict=True,
        )
    ...
    if args.model_disc_frame:
        disc_frame.load_state_dict(
            torch.load(
                os.path.join(args.model_disc_frame, "disc_frame.pt"),
                map_location="cuda" if args.cuda else "cpu",
            ),
            strict=True,
        )
    ...
    tface_trainer = trainer.tfaceTrainer(
        args,
        generator=generator,
