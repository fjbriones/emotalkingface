...
generator.load_state_dict(
    torch.load(
        os.path.join(model_path, "generator.pt"),
...
def deNormImg(img):
    img = np.moveaxis(255.0 * img, 0, 2).astype(
        np.uint8
    )  # np.moveaxis(255.0*(0.5 + (img/2)), 0, 2).astype(np.uint8)
...
with torch.no_grad():
    for s_idx, (h5_file, emotion, speaker) in tqdm(enumerate(h5_file_list)):
        ...
        emo_label = (
            torch.from_numpy(to_categorical(rnd_emo, num_classes=6))
            .float()
            .unsqueeze(0)
        )
        ...
        utils.write_video_cv(
            ...
            args.fs,
            output_path,
            "{}_{}_{}.mp4".format(s_idx, emotion_dict[emotion], emotion_dict[rnd_emo]),
            25.0,
        )
